...
if (!index.includes("APP_RELEASE = 'MOBILE-R1.4.8-LOAD-RESTORE-HOTFIX'")) fail('APP_RELEASE is not Mobile R1.4.8');
...
if (!sw.includes('employee-registry-mobile-r148-2026.09.12')) fail('service worker cache name is not Mobile R1.4.8');
...
if (!String(manifest.description || '').includes('Mobile R1.4.8')) fail('manifest description was not updated');
...
ok(`Mobile R1.4.8 verified: ${version.version}, employees=${version.totalCount}, modified=${summary.counts.modified}`);
