...
const expectedRelease = 'MOBILE-R1.4.63-SPLASH-LOADING-RESTORE';
if (release !== expectedRelease) fail(`VERSION.txt mismatch: ${release}`);
if (!index.includes(`APP_RELEASE = '${expectedRelease}'`)) fail('APP_RELEASE is not Mobile R1.4.63 Splash Loading Restore');
if (!String(manifest.description || '').includes('R1.4.63')) fail('manifest description was not updated to R1.4.63');
...
// R1.4.63 Splash Loading Restore verification
(function verifyR1463(){
  const idx = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
  const ver = fs.readFileSync(path.join(root, 'VERSION.txt'), 'utf8').trim();
  if (ver !== 'MOBILE-R1.4.63-SPLASH-LOADING-RESTORE') fail('R1.4.63 VERSION.txt mismatch');
  ...
  if (!idx.includes('MOBILE-R1.4.63-SPLASH-LOADING-RESTORE')) fail('R1.4.63 APP_RELEASE missing');
  ...
  ok('Mobile R1.4.63 Splash Loading Restore verified: loading screen restored with guarded unlock');
})();
