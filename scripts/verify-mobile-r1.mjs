...
if (!index.includes("APP_RELEASE = 'MOBILE-R1.2-SEARCH-PROFILE-UX'")) fail('APP_RELEASE is not Mobile R1.2');
if (!index.includes('mobile-r11-home') || !index.includes('MOBILE R1.2') || !index.includes('r12-scope-stack')) fail('Mobile R1.2 UI block is missing');
...
if (!sw.includes('employee-registry-mobile-r12-2026.09.11')) fail('service worker cache name is not Mobile R1.2');
...
ok(`Mobile R1.2 verified: ${version.version}, employees=${version.totalCount}, modified=${summary.counts.modified}`);
