...
if (!index.includes("APP_RELEASE = 'MOBILE-R1.4.4-FINAL-READABILITY-LAYOUT-POLISH'")) fail('APP_RELEASE is not Mobile R1.4.4');
for (const marker of ['Mobile R1.4.4 - Final Readability & Layout Polish','r14-hero','r14-emp-card','r14-card-inner','--r141-title','--r141-side','Mobile R1.4.4 - Final Readability & Layout Polish','--r144-title-size','--r144-num-md','mobileR14BuildHome','mobileR14ProfileHero','renderQuickSearch=function','تحديث التطبيق والبيانات']) {
  if (!index.includes(marker)) fail(`Mobile R1.4.4 marker missing: ${marker}`);
}
if (!index.includes('اضغط لعرض التفاصيل الكاملة داخل بطاقة الموظف')) fail('focused employee card hint is missing');
if (!sw.includes('employee-registry-mobile-r144-2026.09.11')) fail('service worker cache name is not Mobile R1.4.4');
...
if (!String(manifest.description || '').includes('Mobile R1.4.4')) fail('manifest description was not updated');
...
ok(`Mobile R1.4.4 verified: ${version.version}, employees=${version.totalCount}, modified=${summary.counts.modified}`);
