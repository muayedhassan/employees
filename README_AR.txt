# HRMobileRepo — Mobile R1.4.30 PDF Browser-Shaped Arabic

حل جذري للعربية داخل بطاقة الموظف PDF: تم إخراج النص العربي بالكامل من محرك jsPDF، وأصبحت البطاقة تُرسم بواسطة محرك Android WebView/Canvas 2D نفسه ثم تُحفظ كصورة PNG عالية الدقة داخل صفحة A4 واحدة. هذا يمنع مشاكل تشكيل الحروف وقلب RTL الناتجة عن jsPDF مع الخطوط العربية المخصصة. يبقى التنزيل المباشر إلى Downloads كما هو. لا تغيير على التقارير أو مركز التحديثات أو البيانات أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.29 PDF Arabic Typography Fix

إصلاح حصري لخطوط بطاقة الموظف PDF: منع التشكيل العربي المزدوج داخل jsPDF، استخدام SFSultan الكامل لأشكال الحروف العربية داخل PDF، والإبقاء على Stencil للأرقام والتواريخ. لا تغيير على التنزيل أو البيانات أو التقارير أو مركز التحديثات أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.27 PDF Vector No Canvas

إصلاح جذري لتصدير بطاقة الموظف PDF على Android: تم إلغاء التقاط DOM/Canvas من مسار التحميل بالكامل، وأصبحت البطاقة تُرسم مباشرة داخل jsPDF من بيانات الموظف بخطوط المشروع وألوان مؤسسية ثم تُحفظ عبر Median إلى Downloads. المعاينة الحالية بقيت كما هي. لا تغيير على التقارير أو مركز التحديثات أو البيانات أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.26 PDF Direct Canvas

إصلاح مركز ومحصور لمشكلة بطاقة الموظف PDF البيضاء على Android WebView. بقي مسار التنزيل المباشر إلى Downloads كما هو بعد ضبط Median، لكن توليد الملف تغير: يتم التقاط بطاقة الموظف مرة واحدة إلى Canvas ثم إدراج الصورة مباشرة داخل jsPDF في صفحة A4 واحدة، بدون مرحلة التقاط ثانية كانت تنتج صفحة بيضاء. لا تغيير على التقارير أو مركز التحديثات أو البيانات أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.25 Reports Polish

صقل واجهة استوديو التقارير فوق R1.4.24: إزالة رسالة الحالة التي تظهر بعد تشغيل التطبيق، جعل تاريخ آخر تحديث بالأرقام الإنجليزية، توحيد أيقونات قوالب التقارير بالذهبي المضيء، جعل «عنوان التقرير» و«النطاق الحالي» قابلين للتعديل ويظهران مباشرة في المعاينة والتصدير، وتمييز الأعمدة المفعلة بالذهبي المضيء وغير المفعلة بحالة خافتة واضحة. لا تغيير على البيانات أو Windows Master Bridge أو مركز التحديثات.

# HRMobileRepo — Mobile R1.4.24 Reports Studio Redesign

إعادة هيكلة كاملة لقسم التقارير: واجهة احترافية، قوالب تقارير، تخصيص الأعمدة، معاينة رسمية، Excel منسق، وPDF/طباعة A4 بهوية مؤسسية وخطوط المشروع. لا تغيير على البيانات أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.23 Updates + Quality Merge

دمج مركز جودة البيانات داخل مركز التحديثات بدون تغيير بنية مركز التحديثات الحالية. زر جودة البيانات المستقل أزيل من التنقل، والنواقص بقيت في تبويبها الحالي، بينما أضيفت فقط الفحوص غير المكررة مثل اكتمال البيانات والتكرار المحتمل للرقم الوظيفي ورقم الهوية. لا تغيير على data/ أو Windows Master Bridge أو مسار PDF في هذا الإصدار.

# HRMobileRepo — Mobile R1.4.21 PDF Fit Worker Hotfix

إصلاح فوق R1.4.20 لمعالجة رسالة «تعذر بدء التنزيل المباشر» التي ظهرت لأن حزمة html2pdf لا تضمن إتاحة html2canvas وjsPDF كمتغيرات عامة داخل WebView. الإصدار الجديد يستخدم html2pdf Worker نفسه لالتقاط البطاقة، ثم يضع الصورة الكاملة داخل إطار A4 آمن ويُنشئ PDF من صفحة واحدة قبل تسليمه إلى جسر Android. لا تغيير على البيانات أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.20 PDF Single Page + Download Status

إصلاح بطاقة الموظف PDF لتظهر كاملة داخل صفحة A4 واحدة دون قص أو انقسام، مع رسالة تنزيل دقيقة على Android. لا تغيير على البيانات أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.19 Native PDF Download Bridge

إصلاح تنزيل بطاقة الموظف على Android WebView: زران مختصران «معاينة البطاقة» و«تحميل البطاقة»، مع تأكيد قبل التحميل ومسار same-origin attachment عبر Service Worker بدل الاعتماد على blob: URL. لا تغيير على البيانات أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.17 PDF View / Direct Download

تطوير صغير فوق R1.4.16 المستقرة: تم فصل بطاقة الموظف PDF إلى زرين مستقلين، «عرض بطاقة PDF» للمعاينة فقط و«تحميل بطاقة PDF» للتنزيل المباشر بعد رسالة تأكيد. لا تغيير على تصميم البطاقة أو البيانات أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.16 Navigation PDF Admin Polish

تطوير فوق R1.4.15 المستقرة: تنقل مقسم واحترافي يمنع تداخل الأقسام، زر حفظ PDF واضح داخل معاينة الهاتف، والإدارة مخصصة لحماية البيانات والاسترجاع فقط. لا تغيير على data/ أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.15 Updates Details Drilldown

تطوير مركز التحديثات فوق R1.4.14 المستقرة: الموظف المعدل يفتح مباشرة على سجل التغييرات، مع ربط أقوى لسجل الموظف وإظهار زر للوصول إلى آخر إصدار يحتوي تغييرات فعلية عندما يكون آخر نشر بلا تغييرات. لا تغيير على data/ أو Windows Master Bridge.

# HRMobileRepo — Mobile R1.4.14 Load Restore Hotfix

إصلاح تحميل R1.4.13 مع الحفاظ على مركز التحديثات النظيف والبطاقات الأربع التفاعلية في الأعلى، دون تغيير بيانات الموظفين أو مسار Windows → GitHub.

# HRMobileRepo — Mobile R1.4.12

هذا الإصدار يصقل بطاقة طباعة الموظف PDF ويثبت قاعدة خطوط الأرقام والتواريخ.

- الأرقام والتواريخ بخط Stencil.
- جهة إصدار الهوية والملاحظات بخط النص العادي.
- الراتب يظهر بالصيغة: الرقم ثم د.ع.
- لا تغيير على البيانات أو الربط مع HRSystem.

# HRMobileRepo — Mobile R1.4.10 Profile Tab Switch Hotfix

إصلاح تبديل تبويبات بطاقة الموظف: الوظيفة، الهوية، الملاحظات، والتغييرات تظهر الآن عند الضغط عليها بدل بقاء البطاقة على نفس المحتوى.

الأمر المقترح بعد النسخ:

```bat
VERIFY_HR_FONTS.cmd
VERIFY_MOBILE_R1.cmd
git add -A
git commit -m "Mobile R1.4.10: profile tab switch hotfix"
git push
```

---

# HRMobileRepo — Mobile R1.4.8 Load Restore Hotfix

إصدار إصلاحي بعد R1.4.7 لاستعادة تحميل البيانات والأرقام عند فتح التطبيق، مع الحفاظ على تصميم الكروت والتفاصيل والخطوط المضمنة.

الأمر المقترح بعد النسخ:

```bat
VERIFY_HR_FONTS.cmd
VERIFY_MOBILE_R1.cmd
git add -A
git commit -m "Mobile R1.4.8: load restore hotfix"
git push
```


## Mobile R1.4.9
صقل واجهة الهاتف بإزالة النص المصغر غير الضروري، توحيد عرض إصدار البيانات، وحذف أزرار التحديث المكررة من الهيدر. التحديث الرسمي يبقى من حالة النظام.


## R1.4.19 — Native PDF Download Bridge
- زر «معاينة البطاقة» للعرض فقط، وزر «تحميل البطاقة» للتنزيل بعد التأكيد.
- على تطبيق Android Median يتم تمرير Blob الـPDF إلى `median.share.downloadFile` بدل الاعتماد على رابط Service Worker فقط.
- يستخدم APK الحالي جسر Median المدمج وBlobDownloader الأصلي، لذلك لا يحتاج هذا التحديث إلى تعديل APK.
- يبقى مسار R1.4.18 كحل احتياطي للمتصفح أو عند غياب الجسر الأصلي.


## R1.4.20 — PDF Single Page + Download Status
- إنشاء البطاقة كصورة كاملة ثم احتواؤها تناسبيًا داخل صفحة A4 واحدة.
- منع قص نصف البطاقة أو تقسيمها بسبب اختلاف أبعاد WebView.
- تحسين حالة التنزيل في APK الحالي ذي التخزين الخاص بالتطبيق.


## R1.4.22 — PDF Blank Page Fix
- إصلاح ملف PDF الأبيض على Android WebView عبر منع التقاط عنصر نهائي موضوع خارج الشاشة.
- يتم تحويل صورة البطاقة الملائمة وهي موجودة ضمن مساحة A4 فعلية ثم تُزال مباشرة بعد إنشاء الملف.
- لا تغيير على البيانات أو Windows Master أو مركز التحديثات.


Mobile R1.4.28 — PDF Library Loader Fix
- إصلاح سبب فشل تحميل البطاقة في R1.4.27: رابط jsPDF 2.5.2 على cdnjs غير متاح.
- استخدام jsPDF 2.5.1 المتاح مع مسار احتياطي عبر unpkg.
- الإبقاء على توليد PDF المتجهي بدون Canvas وعلى الحفظ المباشر في Downloads.

## R1.4.31 — PDF Footer & Salary Polish
- إزالة معلومات المعرّف الثابت وإصدار البيانات ووقت الإنشاء وشارة HRSystem من أسفل بطاقة الموظف PDF.
- توحيد رقم الراتب بخط Stencil نفسه المستخدم لبقية الأرقام، مع إبقاء رمز العملة بخط عربي واضح.
- الحفاظ الكامل على مسار R1.4.30 الناجح للعربية والتنزيل المباشر إلى Downloads.
